Delcampe Web Service - Client - Read Me
+-----
| This is the second version of the Delcampe API.
| 
| The goal of it is to provide a client ready in many language to use the Delcampe API
| |  
...
// There is presently no Sandbox environment on Delcampe !
// All actions are really handled on the production website.
// A sandbox is previewed for future


This is this package, provide a client in PHP, presently the only one available. But tell to provide soon, other language.

If you convert it to another language or technology, please send us a copy, so we can add it in the package 
and try to maintain it for future improvements.

We tried to have this client class self-explaining enough in order for you to integrate it as easy as possible.

We suggest you to begin with the demo of the simpler script: 'server_get_time.php' to become familiar with the global working way, 
then test 'user_get_action.php'

== Package contents ==
 * /client/
  * /client/php/
  * /client/php/withPeclSoap/
  * /client/php/withZendFramework/
  * /client/php/withNuSoap/
